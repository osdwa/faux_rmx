from chaos_track import ChaosTrack

SEED = 1606282789 #2339899427 # 3594052623
TRACKSTART=16394014 #18004836
DAW_BPM = 66
CROSSFADE = 1071  # Exact value, afaik
SAMPLE_RATE = 44100  # Make sure all track have the same SR!
TARGET_LENGTH = 30 * 60 * SAMPLE_RATE

CHAOS = [
    ChaosTrack(
        label="FMS",
        data_dir=r"C:\ProgramData\Spectrasonics\SAGE\SAGE Libraries\User Libraries\ct stuff\ct esa",
        track_name="07 False memory syndrome",
        disable_render=False,

        effect_pan=+0.1,
        effect_stereo_swap=True,  # Stereo-Swap is applied after panning!

        pattern_chance=34./58.,
        repeat_chance=10./58.,
        reverse_chance=31./58.,

        timing_chance=0,
        timing_rush_drag=0,
        timing_range=0,

        speed_main=-6,
        speed_chance=0,
        speed_down_up=0,
        speed_range=0,

        start_idx=0,
        start_position=15354848 - TRACKSTART
    ),

    ChaosTrack(
        label="Contemplation",
        data_dir=r"C:\ProgramData\Spectrasonics\SAGE\SAGE Libraries\User Libraries\ct stuff\ct esa",
        track_name="13 Contemplation",
        disable_render=False,

        effect_pan=+0.3,
        effect_stereo_swap=True,

        pattern_chance=35./58.,
        repeat_chance=12./58.,
        reverse_chance=29./58.,

        timing_chance=0,
        timing_rush_drag=0,
        timing_range=0,

        speed_main=-8,
        speed_chance=0,
        speed_down_up=0,
        speed_range=0,

        start_idx=0,
        start_position=11827009 - TRACKSTART
    ),
     ChaosTrack(
        label="F8",
        data_dir=r"C:\ProgramData\Spectrasonics\SAGE\SAGE Libraries\User Libraries\ct stuff\ct esa",
        track_name="F8 - Mournful cameraderie",
        disable_render=False,

        effect_pan=-0.5,
        effect_stereo_swap=True,

        pattern_chance=58./58.,
        repeat_chance=0./58.,
        reverse_chance=26./58.,

        timing_chance=1,
        timing_rush_drag=.5,
        timing_range=.25,

        speed_main=-8,
        speed_chance=0,
        speed_down_up=0,
        speed_range=0,

        start_idx=0,
        start_position=160362 - TRACKSTART
    ),
       ChaosTrack(
        label="DSFD 3",
        data_dir=r"C:\ProgramData\Spectrasonics\SAGE\SAGE Libraries\User Libraries\ct stuff\ct esa",
        track_name="03 - Part 3",
        disable_render=False,

        effect_pan=+0.2,
        effect_stereo_swap=True,

        pattern_chance=32./58.,
        repeat_chance=0./58.,
        reverse_chance=17./58.,

        timing_chance=0,
        timing_rush_drag=0,
        timing_range=0,

        speed_main=-9,
        speed_chance=0,
        speed_down_up=0,
        speed_range=0,

        start_idx=0,
        start_position=13470645 - TRACKSTART
    )
]
